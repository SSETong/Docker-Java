...
ENV JAVA_VERSION_MAJOR=8 \
    ...
    JAVA_PACKAGE=jdk \
    JAVA_MAIN_DIR=/usr/lib/jdk \
    JAVA_HOME=/usr/lib/jdk/latest 
ENV PATH $PATH:$JAVA_HOME/bin

# Install prepare infrastructure
RUN yum -y install wget tar
# install gcc gcc-c++ make flex bison gperf ruby \
...
RUN set -ex && \
    ...
    mkdir -p ${JAVA_MAIN_DIR} && cd ${JAVA_MAIN_DIR} && \
    tar -zxvf /tmp/java.tar.gz -C ./ && ln -sf jdk1.${JAVA_VERSION_MAJOR}.0_${JAVA_VERSION_MINOR} ${JAVA_HOME} && \
